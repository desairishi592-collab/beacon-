...
// "informational" (neutral gray, info icon). Matches
// schedule-risk-flags-list.tsx / checkin-risk-flags-list.tsx.
...
function RiskFlagCard({ flag }: { flag: EngineeringRiskFlag }) {
  return (
...
function RiskFlagsList({
  ...
  if (flags.length === 0) {
    return (
      <EmptyState
        title="No risk flags for this repository."
        description="Nothing in your most recent GitHub sync crossed a risk threshold."
      />
    )
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-end">
        <label htmlFor="engineering-severity-filter" className="sr-only">
          Filter by severity
        </label>
        <select
          id="engineering-severity-filter"
...
export function RiskFlagsSection({ flags }: { flags: EngineeringRiskFlag[] }) {
  const [severityFilter, setSeverityFilter] = useState<SeverityFilter>('all')

  return (
    <div>
      <h2 className="text-sm font-medium text-neutral-500 dark:text-neutral-400">Engineering flags</h2>
      <RiskFlagsList
        flags={flags}
        severityFilter={severityFilter}
