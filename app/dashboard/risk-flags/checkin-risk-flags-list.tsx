...
function RiskFlagCard({ flag }: { flag: ManualCheckinRiskFlag }) {
  return (
...
function RiskFlagsList({
  ...
  if (flags.length === 0) {
    return (
      <EmptyState
        title="No concerns flagged from your check-ins."
        description="Nothing rated moderate concern or worse in your check-in history."
      />
    )
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-end">
        <label htmlFor="checkin-severity-filter" className="sr-only">
          Filter by severity
        </label>
        <select
          id="checkin-severity-filter"
      ...
      {sortedFlags.length === 0 ? (
        <EmptyState
          ...
          description="Try a different severity, or clear the filter to see all check-in flags."
        />
      ) : (
...
export function RiskFlagsSection({ flags }: { flags: ManualCheckinRiskFlag[] }) {
