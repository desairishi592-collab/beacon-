...
import type { Database, RiskFlag } from '@/lib/supabase/types'
...
// signals, and persist the result as risk_flags rows tied to the upload.
// Re-running for the same upload replaces its existing flags. Takes the
// caller's db client (the manager's own session, not a service-role
...
// schedule_uploads and risk_flags.
export async function analyzeScheduleUpload(
  ...
  const { error: deleteError } = await db.from('risk_flags').delete().eq('upload_id', uploadId)
